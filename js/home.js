$(document).ready(function(){
    let backUrl = "http://localhost:8091/html/login.html";
    let url = "http://localhost:8091/html/details.html";
    let buyUrl = "http://localhost:8091/html/buy.html";
    let sellUrl = "http://localhost:8091/html/sell.html";
    $("#back").click(function() {
        $(location).attr('href',backUrl);
    });
    let dataHTML = getStockDataHTML();

    $("#browse").html(dataHTML);
    ...
    $("#details").click(function() {
        ...
        if(noneSelected) {
            alert("select the stock for details");
        } else {
            $(location).attr('href',url);
        }
    });
    $("#buy").click(function(){
        $(location).attr('href',buyUrl);
    });
    $("#sell").click(function(){
        $(location).attr('href',sellUrl);
    });
});
setTimeout(function(){
    window.location.reload(1);
 }, 30000);
 function getStockDataHTML(){
     let stockData = stockDataFromAPI();
     let stockList = [];
     let html = "";
     $.each(stockData,function(index, stock){
        let div ="<div class='box'>";
        div+="<div><input type='radio' name='stock' id='"+stock.symbol+"' value='"+stock.symbol+"' ></div>"+
        "<div><b>stock symbol: </b>"+stock.symbol+" </div></div><div>" +
        "</div><div>" +
        "<div><b>company: </b>"+stock.companyName+" </div></div><div class='box'>" +
        "<div ><b>exch: </b>"+stock.primaryExchange+"</div>" +
        "<div > <b> open: </b>"+stock.open+"</div>" +
        "<div > <b> latest: </b>"+stock.latestPrice+"</div>" +
        "</div>";
        html+=div;
     });
     return html;

$(document).ready(function(){
    ...
    let data = stockDataFromAPI()[2];

    $("#stknm").html(data.companyName + ":" + data.symbol);
    $("#stkprice").html(data.latestPrice);

    $("#buy").click(function() {
        let ulimit = $("#ulimit").val();
        let qty = $("#qty").val();
        if(ulimit && qty) {
            if(ulimit < data.latestPrice) {
                alert("Upper limit must be > current stock price " + data.latestPrice)
            } else {
            let buyOrderObject = {};
            buyOrderObject["stock"] = data.symbol;
            buyOrderObject["ulimit"] = ulimit;
            buyOrderObject["qty"] = qty;
            buyOrderObject["currprice"] = data.latestPrice;
            ...
            alert("upper limit and qty are mandatory");
        }
    });
